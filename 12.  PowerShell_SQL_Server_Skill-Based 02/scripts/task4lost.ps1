...
"
# Define SQL to insert sample data
$lostArtifactsValues = "
...
try {
    ...
    Write-Host "Inserted $rowsInserted sample records into LostArtifacts table" -ForegroundColor Green

    # Query only the 'Lost' artifacts
    $command.CommandText = "SELECT * FROM LostArtifacts WHERE ArtifactStatus = 'Lost'"
